...
class TestRedisS3Queue:
    def test_counts(self, s3_q):
        ...
        assert s3_q.get_daily_counts(project, day) == 0
        s3_q.update_daily_counts(project)
        assert s3_q.get_daily_counts(project, day) == 1
    ...
    def test_clear(self, s3_q):
        ...
        s3_q.update_daily_counts(project)
        s3_q.update_daily_counts(project)
        assert s3_q.get_daily_counts(project, day) == 2
        past_day = (now - timedelta(days=100)).strftime("%Y-%m-%d")
        s3_q.update_daily_counts(project, day=past_day)
        assert s3_q.get_daily_counts(project, past_day) == 1
        s3_q.clear_daily_counts(project)
        assert s3_q.get_daily_counts(project, past_day) == 0
        assert s3_q.get_daily_counts(project, day) == 2
        s3_q.clear_all_daily_counts()
        assert s3_q.get_daily_counts(project, past_day) == 0
        assert s3_q.get_daily_counts(project, day) == 0
    ...
    def test_pop(self, s3_q):
        ...
        assert s3_q.get_daily_counts(project) == 1
